import{r as u,j as d,e as t,s as x,T as F,w as l,aV as R}from"./index--c24788d7.js";function k(r){const[f,c]=u.exports.useState(0),[g,s]=u.exports.useState(0);return u.exports.useEffect(()=>{const a=()=>{r.current&&(c(r.current.clientWidth),s(r.current.clientHeight))};return a(),window.addEventListener("resize",a),()=>{window.removeEventListener("resize",a)}},[]),{width:f,height:g}}const T=x.div`
  position: fixed;
  /* z-index: ${r=>r.zIndex}; */
  opacity: ${r=>r.opacity};
  left: 50%;
...
`,z=x.canvas`
  width: 100%;
  height: 100%;
`,$=[148,132,89,139,140,177,69,90,235,290],y=["01-hero-lightpass","02-head-bob-turn","03-flip-for-guts","04-explode-tips","05-flip-for-nc","06-transparency-head","07-flip-reveal-guts","08-turn-for-chip","09-scoop-turn","10-fall-into-case"];function L({startY:r,children:f,canvasIdx:c,endY:g}){const s=u.exports.useRef(null),a=u.exports.useRef(null),w=u.exports.useRef(null),{width:p,height:m}=k(a);return u.exports.useEffect(()=>{var v;const h=(v=s.current)==null?void 0:v.getContext("2d"),C=new Image,e=1;C.src=`${b}/${y[c]}/${e.toString().padStart(4,"0")}.jpg`,C.onload=()=>{var A,B;h==null||h.drawImage(C,0,0,(A=s.current)==null?void 0:A.width,(B=s.current)==null?void 0:B.height)},s.current&&(s.current.width=p,s.current.height=m);const o=A=>{var B,E;C.src=`${b}/${y[c]}/${(A<1?1:A).toString().padStart(4,"0")}.jpg`,h==null||h.drawImage(C,0,0,(B=s.current)==null?void 0:B.width,(E=s.current)==null?void 0:E.height)},D=()=>{const A=window.scrollY-r,B=g-r,E=A/B,S=Math.min($[c]-1,Math.floor(E*$[c]));console.log(`
        scrollTop : ${A} (${window.scrollY} - ${r})
        maxScroll : ${B}
        frameIndex : ${S}
      `),w.current=requestAnimationFrame(U=>{o(S)})};return window.addEventListener("scroll",D),()=>{window.removeEventListener("scroll",D),cancelAnimationFrame(w.current)}},[p,m,c]),d(T,{ref:a,zIndex:10-c,opacity:1,children:[t(z,{ref:s}),f]})}const j=x.div`
  color: white;
  font-size: 40px;
  font-weight: 600;
  position: fixed;
